...
app.post('/internal/confirm-bet', async (req, res) => {
  ...
  if (isReplay) {
    console.warn(`[SECURITY] Replay attack detected! Signature: ${txSignature}`);
    return res.status(400).send('Transaction signature already used');
  }

  // Log the transaction immediately to prevent race conditions
  try {
    await logBetTransaction(txSignature, walletAddress, amount);
  } catch (error) {
